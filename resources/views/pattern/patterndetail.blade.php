...
<table class="table table-striped database-tables" border="1">
		<img src="{{ Storage::url('uploads/'.$id.'/'.$id.'-'.$no.'.bmp') }}" alt="Smiley face" height="360" width="900">
	{{-- <tr>
		<th>Desen Adı</th>
		<th>Müşteri</th>
...
<table class="table table-striped database-tables" border="1">
    ...
    <th>Desen</th>
        ...
        @php $noo=null; $s=$pattern->patterndetail->min('variant_no'); 
        $row=$pattern->patterndetail->where('variant_no',$s)->count();  @endphp 
		@foreach ($pattern->patterndetail->where('variant_no',$no)->sortBy('variant_no') as $list)
		<tr>
			@if($noo != $list->variant_no)
      ...
      <td>{{$list->bands->name ?? ''}}</td>
			...
			<td>{{$list->picksrpt ?? ''}}</td>
			<td>{{$list->tp ?? ''}}</td>
        @php $noo=$list->variant_no; $row++; @endphp 
		</tr>
		@endforeach
		@endisset
</table>
		<img src="{{ Storage::url('uploads/'.$id.'/'.$id.'-Draft'.'.bmp') }}" alt="Smiley face" ><br><br>
		<img src="{{ Storage::url('uploads/'.$id.'/'.$id.'-Peg'.'.bmp') }}" alt="Smiley face" >
@endsection    
